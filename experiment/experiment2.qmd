...
results <- list()
for(i in 1:6){
  data <- read.csv(file = file.path(file.path('..', 'Data', paste0('2023ResultsExperiment2Group', i,'.csv'))))
...
The selected mechanism to attribute the reward in Group 1 is Time.
...
The selected mechanism to attribute the reward in Group 2 is DC-5 Lottery.
...
The selected mechanism to attribute the reward in Group 3 is Time.
...
The selected mechanism to attribute the reward in Group 4 is Time.
...
The selected mechanism to attribute the reward in Group 5 is DC-5 Lottery.
...
The selected mechanism to attribute the reward in Group 6 is DC-5 Lottery`.
